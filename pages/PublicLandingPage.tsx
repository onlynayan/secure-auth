...
import { ShieldCheck, UserCog, User, ChevronRight } from 'lucide-react';
...
const PublicLandingPage: React.FC = () => {
  ...
  return (
    <div className="w-full min-h-screen relative bg-slate-950 flex flex-col items-center py-12 px-6">
      {/* Background Slider */}
      ...
      <div className="relative z-10 w-full max-w-4xl flex flex-col items-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 w-full">
          <button
            ...
            className="group relative bg-white/5 backdrop-blur-xl border border-white/10 p-8 md:p-10 rounded-[2rem] md:rounded-[2.5rem] text-left transition-all hover:bg-white/10 hover:border-indigo-500/50 hover:translate-y-[-4px]"
          >
            <div className="w-12 h-12 md:w-14 h-14 bg-indigo-500/20 rounded-2xl flex items-center justify-center mb-6 border border-indigo-500/30 group-hover:scale-110 transition-transform">
              <UserCog className="w-6 h-6 md:w-7 h-7 text-indigo-400" />
            </div>
            <h3 className="text-xl md:text-2xl font-bold text-white mb-2">Admin Portal</h3>
          ...
          <button
            ...
            className="group relative bg-white/5 backdrop-blur-xl border border-white/10 p-8 md:p-10 rounded-[2rem] md:rounded-[2.5rem] text-left transition-all hover:bg-white/10 hover:border-emerald-500/50 hover:translate-y-[-4px]"
          >
            <div className="w-12 h-12 md:w-14 h-14 bg-emerald-500/20 rounded-2xl flex items-center justify-center mb-6 border border-emerald-500/30 group-hover:scale-110 transition-transform">
              <User className="w-6 h-6 md:w-7 h-7 text-emerald-400" />
            </div>
            <h3 className="text-xl md:text-2xl font-bold text-white mb-2">User Access</h3>
            <p className="text-slate-400 text-sm leading-relaxed mb-6">
        ...
        <div className="mt-12 flex gap-3">
          {SLIDES.map((_, i) => (
            <div
              key={i}
              className={`h-1 rounded-full transition-all duration-500 ${i === currentSlide ? 'w-8 bg-indigo-500' : 'w-4 bg-white/20'
                }`}
            />
          ))}
        </div>
